...
def forum_template_path(instance,filename):
    ext = filename.split('.')[-1]
    filename = '{}.{}'.format(uuid.uuid4().hex[:10],ext)
    # return the whole path to the file
    return os.path.join("forum",instance.name,filename)

class Forum_template(models.Model):
    """论坛版块表"""
    name = models.CharField(max_length=64, unique=True, verbose_name="板块名称")
    image = models.ImageField(upload_to=forum_template_path,verbose_name='图标',blank=True)
    # CATEGORY_CHOICES = (
    #     (1, '一级类目'),
    #     (2, '二级类目'),
    #     (3, '三级类目')
    ...
    author = models.ForeignKey(User, verbose_name="版主", on_delete=models.CASCADE)
...
class Forum(models.Model):
    ...
    category = models.ForeignKey(Forum_template, verbose_name='板块名称',on_delete=models.CASCADE)
    ...
    can_view = models.BooleanField(default=False,verbose_name="是否隐藏可见")
...
class Forum_click(models.Model):
    """帖子浏览量"""
    forum = models.ForeignKey(Forum, verbose_name='帖子', on_delete=models.CASCADE)
...
class Parent_Comment(models.Model):
    ...
    to_Parent_Comment = models.ForeignKey(User,on_delete=models.CASCADE,verbose_name='目标用户',related_name='to_Parent_Commenty',default='')
    forum = models.ForeignKey(Forum, verbose_name='帖子', on_delete=models.CASCADE)
    content = models.TextField(verbose_name='评论')
    parent_comment = models.ForeignKey(Comment,blank=True,null=True,on_delete=models.CASCADE)
    address = models.CharField(max_length=50, verbose_name='地址', blank=True, null=True)
    url = models.CharField(max_length=60, blank=True, null=True, default='')
    create_time = models.DateTimeField(default=datetime.now, verbose_name='添加时间')
    def __str__(self):
